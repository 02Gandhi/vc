        ...
        return (
            <div className="bg-brand-background min-h-screen text-brand-text-primary">
                ...
                <main className="container mx-auto p-8 text-center">
                    ...
                    <p className="text-brand-text-secondary mt-4">The contractor profile you are looking for does not exist.</p>
            ...
            <main className="container mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="space-y-6">
                                {profile.portfolio.length > 0 ? profile.portfolio.map(item => (
                                    <div key={item.id} className="border-b border-brand-border pb-6 last:border-b-0">
                                        ...
                                        <p className="text-sm text-brand-text-secondary mb-2">{item.location} • {item.dateStart ? `${new Date(item.dateStart).getFullYear()} - ${new Date(item.dateEnd).getFullYear()}` : ''}</p>
                                        <p className="text-brand-text-primary mb-4">{item.description}</p>
                                ...
                                )) : <p className="text-brand-text-secondary">No portfolio projects have been added yet.</p>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="bg-brand-surface p-6 rounded-lg border border-brand-border">
                            <h3 className="text-xl font-semibold mb-4 text-brand-text-primary">Contractor Details</h3>
                            <div className="space-y-4">
                                <DetailItem label="Location" icon={
                                     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>
                                 }>
                                    {profile.address.city}, {profile.address.country}
                                </DetailItem>
                                <DetailItem label="Team Size" icon={
                                    ...
                                    {profile.team.companySize}+ employees
                                </DetailItem>
                                <DetailItem label="Specialties" icon={
                        ...
                        <div className="bg-brand-surface p-6 rounded-lg border border-brand-border">
                             ...
                             <div className="space-y-4">
                                <p className="text-sm text-brand-text-secondary">Contact details are available to clients after purchase on the project page.</p>
                                <DetailItem label="Contact Person" icon={
